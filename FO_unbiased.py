...
# baseline
Agent1_baseline1 = DQNAgent(budget=budget, state=[
                            0, 0, 0, 0], observation_space=4, action_space=action_space1)
# state
Agent2_operation1 = DQNAgent(budget=budget, state=[
                             0, 0, 0, 0, 0], observation_space=5, action_space=action_space1)
# action
Agent3_operation3 = DQNAgent(budget=budget, state=[
                             0, 0, 0, 0], observation_space=4, action_space=action_space2)
# reward
Agent4_operation2 = DQNAgent(budget=budget, state=[
                             0, 0, 0, 0], observation_space=4, action_space=action_space1)
Agents = [Agent1_baseline1, Agent2_operation1,
          Agent3_operation3, Agent4_operation2]
...
for time in range(10):
    ...
    for i in range(4):
        ...
        if i == 1:
            Agents[i].state = [
                budget, budget_consumption_rate, win_rate, step, 0]
        else:
...
# Our_client.network.load_state_dict(checkpoint)
    ...
    for request in range(1, n_request_left + 1):
        ...
        for i in range(4):
            ...
            if i == 2:  # for agent which take lamda as action
                if random.random() > epsilon:
                    ...
                    temp_price = Agents[i].get_price_unbiased(
                        unique_bid, win_prob, l)  # For agent3
                else:
        ...
        for i in range(4):
            if bid_p[i] == np.max(bid_p):
                ...
                if i == 1 or i == 2 or i == 3:
                    Agents[i].update_w_dw(
                        bid_price=bid_p[i], flag=1, request=request)
                ...
                if i == 1 or i == 2 or i == 3:
                    Agents[i].update_w_dw(
                        bid_price=bid_p[i], flag=0, request=request)
                global_wi.append(0)
                global_zi.append(0)
                reward_1.append(-1)
        ...
        unique_bid, win_prob = win_prob_second_list(
            global_bi, global_wi, global_zi)
        # print(unique_bid,win_prob)

        for i in range(4):
            win_rate = np.sum(Agents[i].win_rate) / \
                (len(Agents[i].win_rate)+0.001)
            ...
            if len(Agents[i].state) == 5:  # for agent 2, using unbiased model
                Agents[i].next_state[4] = Agents[i].get_lambda_unbiased(
                    unique_bid, win_prob, bid_p[i], theta)
        ...
        for i in range(4):
            if Agents[0].budget < 100 and Agents[1].budget < 100 and Agents[2].budget < 100 and Agents[3].budget < 100:
                ...
                if i == 3:
                    ...
                    Agents[i].reward = reward_1[i] + 500 * \
                        Agents[i].get_lambda_unbiased(
                            unique_bid, win_prob, bid_p[3], theta)
                    if request % 1000 == 0:
                        print(
                            500*Agents[i].get_lambda_unbiased(unique_bid, win_prob, bid_p[3], theta))
                Agents[i].reward = reward_1[i]

        for i in range(4):
            Agents[i].replayBuffer.push(
                Agents[i].state, Agents[i].action, Agents[i].reward, Agents[i].next_state, done)
        ...
        if request % batch_size == 0:
            for i in range(4):
                loss = compute_td_loss(
                    Agents[i].network, Agents[i].optimizer, Agents[i].replayBuffer, gamma, batch_size)
            if Agents[0].budget < 100 and Agents[1].budget < 100 and Agents[2].budget < 100 and Agents[3].budget < 100:
                break
